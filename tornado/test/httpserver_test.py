...
class HTTPConnectionTest(AsyncHTTPTestCase):
    ...
    def test_multipart_form(self):
        ...
        response = self.raw_fetch([
            "POST /multipart HTTP/1.0",
            "Content-Type: multipart/form-data; boundary=1234567890",
            "X-Header-encoding-test: \xe9",
        ],
            bytes_type("\r\n").join([
            "Content-Disposition: form-data; name=argument",
            "",
            u("\u00e1").encode("utf-8"),
            "--1234567890",
    ...
    def test_100_continue(self):
        ...
        stream.read_until("\r\n\r\n", self.stop)
        data = self.wait()
        self.assertTrue(data.startswith("HTTP/1.1 100 "), data)
        stream.write("a" * 1024)
        stream.read_until("\r\n", self.stop)
        first_line = self.wait()
        self.assertTrue(first_line.startswith("HTTP/1.1 200"), first_line)
        stream.read_until("\r\n\r\n", self.stop)
...
class KeepAliveTest(AsyncHTTPTestCase):
    ...
    def read_headers(self):
        ...
        first_line = self.wait()
        self.assertTrue(first_line.startswith(self.http_version + ' 200'), first_line)
        self.stream.read_until('\r\n\r\n', self.stop)
    ...
    def test_cancel_during_download(self):
        self.connect()
        self.stream.write('GET /large HTTP/1.1\r\n\r\n')
        self.read_headers()
    ...
    def test_finish_while_closed(self):
        self.connect()
        self.stream.write('GET /finish_on_close HTTP/1.1\r\n\r\n')
        self.read_headers()
        self.close()
